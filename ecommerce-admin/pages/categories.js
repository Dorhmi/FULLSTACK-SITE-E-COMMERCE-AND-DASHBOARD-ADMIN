...
function Categories({swal}) {
    ...
    function handlePropertyValuesChange(index,property,newValues) {
      ...
      function removeProperty(indexToRrmove) {
        setProperties(prev => {
            return [...prev].filter((p,pIndex) => {
                return pIndex !== indexToRrmove ;
            });
        });
      }
    return(
    <Layout>
        ...
        <form onSubmit={saveCategory} >
            ...
            <input onChange={ev => setName(ev.target.value)} value={name}  type="text" placeholder={'Categorie name'}></input>
            ...
            <div className="mb-2">
                ...
                {properties.length > 0 && properties.map((property,index) => (
                    <div className="flex gap-1 mb-2">
                        <input className="mb-0" type="text" value={property.name} onChange={ev => handlePropertyNameChange(index,property,ev.target.value)} placeholder="property name (example: color)"></input>
                        <input className="mb-0" type="text" value={property.values} onChange={ev => handlePropertyValuesChange(index,property,ev.target.value)} placeholder="values, comma separated"></input>
